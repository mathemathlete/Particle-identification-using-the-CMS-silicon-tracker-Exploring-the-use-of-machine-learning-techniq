...
def preparation_data(file_in,file_out,branch_of_interest):
    """Préparation du fichier sans filtrage"""
    data = pd.DataFrame()
...
def preparation_data2(data,file_out):
...
if __name__ == "__main__":  
    ...
    # filtred_data = cpf.filtrage_dedx("Root_files/tree.root",["dedx_charge", "dedx_pathlength", "track_p","track_eta"],True,True,True)
    # preparation_data2(filtred_data,"ML_training_LSTM_filtré.root")

    data_plot = cpf.filtrage_dedx("Root_Files/signal.root",["dedx_charge", "dedx_pathlength", "track_p","track_eta"],True,True,True)
    preparation_data2(data_plot,"Signal_Plot.root")
